...
namespace Prova1.Infra.ORM.Tests.Features.Orders
{
    ...
    public class OrderRepositoryTests : EffortTestBase
    {
        ...
        [Test]
        public void Orders_Repository_GetById_ShouldThrowNotFoundException()
        {
            //Action
            var notFoundId = 10;
        ...
        {
            // Action
            var isRemoved = _repository.Remove(_orderSeed.Id);
            // Assert
        ...
        {
            // Arrange
            _order = ObjectMother.GetOrderValid(); // Precisamos de um novo: o do SetUp está indexado
            var unknownId = 20;
            _order.Id = unknownId;
            //Action
            Action updatedAction = () => _repository.Update(_order);
            // Verify
            updatedAction.Should().Throw<InvalidOperationException>();
        }

        [Test]
        public void Orders_Repository_Delete_ShouldHandleUnknownProductId()
        {
            // Arrange
