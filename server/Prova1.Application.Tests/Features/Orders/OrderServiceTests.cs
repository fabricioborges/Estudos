...
namespace Prova1.Application.Tests.Features.Orders
{
    ...
    public class OrderServiceTests : TestServiceBase
    {
        ...
        public void Orders_Service_Add_ShouldBeOk()
        {
            ...
            _orderRepositoryFake.Setup(odr => odr.Add(order)).Returns(order);
            _productRepositoryFake.Setup(odr => odr.GetById(order.Product.Id)).Returns(order.Product);
            //Action
            var newCustomerId = _service.Add(order);
            //Assert
            _orderRepositoryFake.Verify(odr => odr.Add(order), Times.Once);
        ...
        public void Orders_Service_Add_ShouldBeHandleNotFoundExceptionInProduct()
        {
            //Arrange
            var order = ObjectMother.GetOrderValidToRegister();
            _productRepositoryFake.Setup(odr => odr.GetById(order.Product.Id)).Returns((Product)null);
            // Aqui usamos genérico para simular uma exceção qualquer (de banco, etc) disparada pelo repositório
            _orderRepositoryFake.Setup(odr => odr.Add(order)).Throws<NotFoundException>();
            //Action
            Action newCustomerAction = () => { _service.Add(order); };
            //Assert
            newCustomerAction.Should().Throw<NotFoundException>();
            _orderRepositoryFake.Verify(odr => odr.Add(order), Times.Never);
        }
        ...
        public void Orders_Service_Add_ShouldBeHandleException()
        {
            ...
            _productRepositoryFake.Setup(odr => odr.GetById(order.Product.Id)).Returns(order.Product);
            // Aqui usamos genérico para simular uma exceção qualquer (de banco, etc) disparada pelo repositório
            _orderRepositoryFake.Setup(odr => odr.Add(order)).Throws<Exception>();
            var newOrderId = 0;
            //Action
            Action newCustomerAction = () => { newOrderId = _service.Add(order); };
            //Assert
        ...
        public void Orders_Service_GetAll_ShouldBeOk()
        {
            ...
            //Action
            var orderResult = _service.GetAll();
            //Assert
        ...
        {
            //Arrange
            var order = ObjectMother.GetOrderValidToRemove();
            var mockIsRemoved = true;
            _orderRepositoryFake.Setup(odr => odr.Remove(order.Id)).Returns(mockIsRemoved);
            //Action
            var isOrderRemoved = _service.Remove(order);
            //Assert
            _orderRepositoryFake.Verify(odr => odr.Remove(order.Id), Times.Once);
        ...
        public void Orders_Service_Delete_ShouldBeHandleNotFoundException()
        {
            ...
            var order = ObjectMother.GetOrderValidToRemove();
            var exception = new NotFoundException();
            _orderRepositoryFake.Setup(odr => odr.Remove(order.Id)).Throws(exception);
            //Action
            Action orderAction = () => _service.Remove(order);
            //Assert
            orderAction.Should().Throw<NotFoundException>();
            // Perceba que nesse caso estamos simulando um cenário onde o repositório
            // reporta uma exception - logo ele deve ser chamado
        ...
        public void Orders_Service_Update_ShouldBeOk()
        {
            ...
            //Action
            var isOrderRemoved = _service.Update(order);
            //Assert
            _orderRepositoryFake.Verify(odr => odr.GetById(order.Id), Times.Once);
